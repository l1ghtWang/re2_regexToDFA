...
// std::string benchmarkPath = "/home/yugwang/re2_regexToDFA/ANMLZoo_regex/powerEN/powerEN_";
std::string benchmarkPath = "/home/yugwang/re2_regexToDFA/ANMLZoo_regex/yara/yara_";
...
TEST(SingleThreaded, BuildEntireDFA) {
  ...
  std::cout<<s<<std::endl;
//   std::string s2 = "(\\/[^\\r\\x8a]*\\x20.pl)|(mesajno=[^\\x8c]*?(SELECT[^&]*?FROM|UPDATE[^\\x8b]*?SET))|(lang=[^\\s\\x3b\\x2c]*?(\\x2e\\x2e|%2e%2e)(\\x2f|%2f))|(^Host\\x3a[^\\r\\n]*ad\\x2Emokead\\x2Ecom)";

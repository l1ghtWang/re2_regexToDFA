...
/* ---------class Input------------ */
    ...
    std::string inputFileName("/home/share/powerEN/powerEN_input_1200MB_2.in");
	...
	int nSymbol_;
	int numStates_;
	int num_correctSpecChunk = 0;
	int num_recoveryWholeChunk = 0;
	long sum_convergentLength = 0;
	...
	int MostFrequent(int arr[], int n)
	{
		...
		// If every element has the same frequency
		if (max_count == 1)
		{
			srand (time(NULL));
			res = rand() % n;
...
void generateFSM_profiling_outputToFile(std::vector<std::string> regex_list, int num_profile_point, Input* inputs_){
	...
	nSymbol_ = 256;
	numStates_ = table.size()/256;
	std::cout<<"table.size(): "<<table.size()<<std::endl;
	std::cout<<"numStates_: "<<numStates_<<std::endl;
	if(numStates_ > 10000)
	{
		delete prog;
		re->Decref();
	...
	vector<long> entryCounter(numStates_*nSymbol_, 0);
	...
	for (long i = 0; i < length_; i++)
	{
		...
		if (cur_point < rand_checkpoints.size() && i == rand_checkpoints[cur_point])
		{
			...
			spec_at(numStates_, nSymbol_, table, inputs_, i, ranked_states);
	...
	if(averageConvergenceLen > 50000000 && num_10_8levelFSM < targetNum_10_8levelFSM)
	{
		...
		writeTableAC_toFile(dfa_index, numStates_, nSymbol_, table, ac, averageConvergenceLen,leastNumTableEntry_percent90TotalTransition, combRegex);
		num_10_8levelFSM += 1;
...
struct thread_data{
	int thread_id;
	std::vector<int> *table_vec;
	int numStates;
	int nSymbol;
	Input* input_ptr;
};
int mSpecs[NUM_THREADS];
int mEnds[NUM_THREADS];
struct thread_data thread_data_array[NUM_THREADS];
int mFinalEndState = mEnds[NUM_THREADS-1];
int mNum_correctSpecChunk = num_correctSpecChunk;
int mNum_recoveryWholeChunk = num_recoveryWholeChunk;
int mAve_convgentLength = sum_convergentLength*1.0/(NUM_THREADS -1);

void* callFunc_parallelRun(void* args)
{
	struct thread_data *my_data;
	my_data = (struct thread_data *) args;
	int tid = my_data->thread_id;
	std::vector<int> &tableList_ = *(my_data->table_vec);
	const Input* input = my_data->input_ptr;

	int nstate_ = my_data->numStates;
	int nsymbol_ = my_data->nSymbol;
	
	uint8_t* inputs_ = input->getHostPointer();

	long length_ = input->getLength();

	int state_;
	long ChunkLength_ = length_ / NUM_THREADS;
	long startIndex = tid * ChunkLength_;
	long endIndex = (tid + 1) * ChunkLength_;

	#ifdef ACTION
		PhiFunction* functionP = this->getAction(); 
	#endif			
	...
	{
		unsigned char cur_symbol = inputs_[i];
		state_ = tableList_[state_ * nsymbol_ + cur_symbol];

		// Action Function (state_)
		#ifdef ACTION
			functionP->parallelExecution(state_, table, i, tid);
		#endif
	}		
...
void re_execute(std::vector<int> &tableList_, Input *input)
{
	...
	uint8_t* inputs_ = input->getHostPointer();
	long length_ = input->getLength();
	long chunkLength = length_/NUM_THREADS;

	#ifdef ACTION
		PhiFunction* functionP = this->getAction(); 
	#endif			
	
	num_correctSpecChunk = 0;
	num_recoveryWholeChunk = 0;
	sum_convergentLength = 0;
	int i;
	for (i = 1; i < NUM_THREADS; i++)
	{
		if (mSpecs[i] != mEnds[i-1])
		{
			int correctState = mEnds[i-1];
			int predictState = mSpecs[i];
			long j;
			for (j = i*chunkLength; j < (i+1)*chunkLength; j++)
			{
				unsigned char cur_symbol = inputs_[j];
				correctState = tableList_[correctState * nSymbol_ + cur_symbol];
				predictState = tableList_[predictState * nSymbol_ + cur_symbol];
				sum_convergentLength++;
				// insert Action Function (correctState, predictState)
				#ifdef ACTION
					functionP->reExecution(correctState, predictState, table, j, i);
				#endif
			...
			{
				num_recoveryWholeChunk++;
				mEnds[i] = correctState;
			}
		}
		else
			num_correctSpecChunk++;
	}

	#ifdef ACTION
		functionP->parallelMerge();
	#endif

	// To handle the rest input contents
	if (input->getLength() > (chunkLength*NUM_THREADS))
	{
		int restStartState = mEnds[NUM_THREADS-1];
		long restStartIndex = chunkLength*NUM_THREADS;
		for (i = restStartIndex; i < input->getLength(); i++)
		{
			restStartState = tableList_[restStartState * nSymbol_ + inputs_[i]];

			#ifdef ACTION
				functionP->execution(restStartState, table, i);
			#endif
		}
		mEnds[NUM_THREADS-1] = restStartState;
	}
	std::cout<<"real recovery length: "<<sum_convergentLength<<std::endl;
	mFinalEndState = mEnds[NUM_THREADS-1];
	mNum_correctSpecChunk = num_correctSpecChunk;
	mNum_recoveryWholeChunk = num_recoveryWholeChunk;
	mAve_convgentLength = sum_convergentLength*1.0/(NUM_THREADS -1);
}


void generateFSM_profilingRealConvLen_outputToFile(std::vector<std::string> regex_list, Input* inputs_)
{
	long length_ = inputs_->getLength();
	uint8_t* inputStream = inputs_->getHostPointer();
	...
	long ChunkLength_ = length_/numThread;
	int nstate_ = numStates;
	// int mAllSpecStates[numThread*nstate_];
	int *mAllSpecStates = new int[numThread*nstate_];
	...
	for(int tid=1; tid<numThread;tid++)
	{
		...
		for (int j = 0; j < nstate_; j++)
			mAllSpecStates[tid*nstate_+j] = j; 

		long startIndex = tid * ChunkLength_;
		long endIndex = (tid + 1) * ChunkLength_;
		int offset_SpecState = nstate_ * (tid-1);
		int* cur_SpecStates = &mAllSpecStates[offset_SpecState];
		for (long i = startIndex - mLookBack; i < startIndex; i++)
		{
			unsigned char cur_symbol_suffix = inputStream[i];
			for (int j = 0; j < nstate_; j++)
				cur_SpecStates[j] = table[cur_SpecStates[j] * nSymbol_ + cur_symbol_suffix];
		}
		mSpecs[tid] = MostFrequent(&mAllSpecStates[offset_SpecState], nstate_);
	}
	pthread_t* threads;
	...
	cpu_set_t cpus;		
	int m_MaxCPU = std::thread::hardware_concurrency();
	...
	{
		CPU_ZERO(&cpus);
		int numPhyCore = m_MaxCPU/2;
		int coreID_toAssign = numPhyCore*(t/(numPhyCore-1)) + t%(numPhyCore-1);//chunk-logicalCore mapping, skip logical core47 in current machine
		// cout<<"coreID_toAssign:"<<coreID_toAssign<<endl;
		// cout<<"chk_id:"<<t<<endl;
		CPU_SET(coreID_toAssign, &cpus);
		pthread_attr_setaffinity_np(&attr, sizeof(cpu_set_t), &cpus);	
		thread_data_array[t].thread_id = t;
		thread_data_array[t].table_vec = &table;
		thread_data_array[t].numStates = nstate_;
		thread_data_array[t].nSymbol = nSymbol_;
		thread_data_array[t].input_ptr = inputs_;
		pthread_create(&threads[t], &attr, callFunc_parallelRun, (void*)&thread_data_array[t]);
						
	}
	for(t=0; t< numThread; t++)
	    	pthread_join(threads[t], NULL);	

	re_execute(table, inputs_);



	if(sum_convergentLength > 50000000 && num_10_8levelFSM < targetNum_10_8levelFSM)
	{
		int dfa_index = dfa_baseIdx+num_10_8levelFSM;
		writeTableAC_toFile(dfa_index, numStates, nSymbol_, table, ac, sum_convergentLength,0, combRegex);
		num_10_8levelFSM += 1;
		cout<<"num_10_8levelFSM: "<<num_10_8levelFSM<<endl;
	}
	else if(sum_convergentLength <= 50000000 && sum_convergentLength > 5000000 && num_10_7levelFSM < targetNum_10_7levelFSM)
	{
	...
	else if(sum_convergentLength <= 5000000 && sum_convergentLength > 500000 && num_10_6levelFSM < targetNum_10_6levelFSM)
	{
		int dfa_index = dfa_baseIdx+targetNum_10_8levelFSM+targetNum_10_7levelFSM+num_10_6levelFSM;
		writeTableAC_toFile(dfa_index, numStates, nSymbol_, table, ac, sum_convergentLength,0, combRegex);
		num_10_6levelFSM += 1;
	...
	else if(sum_convergentLength <= 500000 && sum_convergentLength > 50000 && num_10_5levelFSM < num_10_5levelFSM)
	{
		int dfa_index = dfa_baseIdx+targetNum_10_8levelFSM+targetNum_10_7levelFSM+targetNum_10_6levelFSM+num_10_5levelFSM;
		writeTableAC_toFile(dfa_index, numStates, nSymbol_, table, ac, sum_convergentLength,0, combRegex);      
		num_10_5levelFSM += 1;
...
void generateFSM_profilingRealConvLen_outputToFile_wrapper(std::vector<std::string> regex_list, Input* inputs_)
{
    std::mutex m;
    std::condition_variable cv;
    int retValue;

    std::thread t([&cv, &regex_list, &inputs_]() 
    {
        generateFSM_profilingRealConvLen_outputToFile(regex_list, inputs_);
        cv.notify_one();
    });
...
TEST(SingleThreaded, BuildEntireDFA) {
	...
	{
		// if(whileCounter == 1) exit(0);///////////////developing, delete this line later
		...
		bool timedout = false;
		try {
			generateFSM_profilingRealConvLen_outputToFile_wrapper(regex_list, inputs_);
		}

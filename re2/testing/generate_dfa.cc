...
std::string regexFileNameTemp("/home/yugwang/re2_regexToDFA/build/temp.regex");
// std::string regexFileNameTemp("/home/yugwang/re2_regexToDFA/ANMLZoo_regex/snort_regex");
// std::string inputFileName("/home/wygzero/re2/ANMLZoo_regex/clamAV/clamAV_vasim_10MB.input");
    // std::string pattern_str = "(a[^\x26]*\x2E\x2E)|(a[^\x3b]*\x3b.*\x2ejsp)";
    // std::string pattern_str = "(\/[^\r\\x8a]*\\x20.pl)|(mesajno=[^\\x8c]*?(SELECT[^&]*?FROM|UPDATE[^\\x8b]*?SET))|(lang=[^\s\\x3b\\x2c]*?(\\x2e\\x2e|%2e%2e)(\\x2f|%2f))|(^Host\\x3a[^\r\n]*ad\\x2Emokead\\x2Ecom)";
    // std::string pattern_str = "(/test\\x2Eexe)";
    std::string pattern_str = "\.{4}";
TEST(SingleThreaded, BuildEntireDFA) {


    pattern_str = readFileIntoString(regexFileNameTemp);
    std::cout<< pattern_str <<std::endl;
    // std::string s = "(^[^\x26]*?\x2E\x2E)|(^[^\x3b]*\x3b.*\x2ejsp)";
    // std::string s = "(a[^\x26]*\x2E\x2E)|(a[^\x3b]*\x3b.*\x2ejsp)";
    // std::string s = "(a[^b]*c)|(a[^k]*d)";

    // std::string s = "(aba)|(.xcd)|(.{1,20})|(([A-Za-z0-9+\x2f]{4})*([A-Za-z0-9+\x2f]{2}==|[A-Za-z0-9+\x2f]{3}=)?)";

    Regexp* re = Regexp::Parse(pattern_str, Regexp::LikePerl|Regexp::Latin1, NULL);
...
TEST(SingleThreaded, checkhDFA) {
  ...
  // {
  //   std::cout<<"hello and h.*o matched"<<std::endl;
  // }
  // CHECK(RE2::FullMatch("4a", "[0-9]"));


  // std::string pattern_str = readFileIntoString(regexFileNameTemp);
  // std::string pattern_str = "\\x61\\x62\\x6a";

  // std::string pattern_str = "(a[^b]*c)|(a[^k]*d)";
  ...
  // char input_str [2] = {(char)97, (char)200};
  // char input_str [] = {(char)194,(char)138,0};
  // char input_str [] = {'0x8a', '0xb1'};
  // char input_str [] = {97, 0};
  // char input_str [] = {(char)138, '0'};
//   char input_str [] = "aaaaaaaa";
  char input_str [] = "abcd";

  std::cout<< "input_str:"<<input_str<<std::endl;
  // std::string contents = "\x8F";
  StringPiece input(input_str);
  if(RE2::FullMatch(input_str, pattern_str.c_str()))
  {
    std::cout<<input_str<<" and "+pattern_str+" matched"<<std::endl;
  }
  else
  {
      std::cout<<input_str<<" and "+pattern_str+" NOT matching!!!"<<std::endl;
  }
  // CHECK(RE2::FullMatch(input, pattern));
